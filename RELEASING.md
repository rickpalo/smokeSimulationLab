...
> Repository slug and folder name remain `SmokeSimLab` (v0.6.3 surface-only rebrand).  GitHub release zip artifact is `SmokeSimLab.zip`.
...
```sh
blender --command extension validate scripts/SmokeSimLab
...
```sh
blender --command extension build --source-dir scripts/SmokeSimLab --output-dir dist
...
- The repo slug stays `SmokeSimLab` (the v0.6.3 rebrand was surface-only); the
  extension `id` is `batchsimlab` and must not drift (it keys the install dir).
